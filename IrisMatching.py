...
def selectTestSample(test_features, test_classes):
    index = random.sample(xrange(len(test_classes)), 108)
...
def CalcTest(train_features, train_classes, test_sample, test_class, dist):
    ...
    for i in xrange(len(train_classes)):
        if dist == 1:
            distoff = np.ones(11)
...
def calcROCBootstrap(fmrs, fnmrs):
    ...
    fmrs_std = np.std(fmrs, axis = 0)
    l_tmp = fmrs_mean - fmrs_std*1.96
    fmrs_l = np.array([max(l_tmp[l],0) for l in xrange(len(l_tmp))])
    u_tmp = fmrs_mean + fmrs_std*1.96
    fmrs_u = np.array([min(u_tmp[u],1) for u in xrange(len(u_tmp))])
    
    fnmrs_mean = np.mean(fnmrs, axis = 0)
    fnmrs_std = np.std(fnmrs, axis = 0)
    l_tmp = fnmrs_mean - fnmrs_std*1.96
    fnmrs_l = np.array([max(l_tmp[l],0) for l in xrange(len(l_tmp))])
    u_tmp = fnmrs_mean + fnmrs_std*1.96
    fnmrs_u = np.array([min(u_tmp[u],1) for u in xrange(len(u_tmp))])
